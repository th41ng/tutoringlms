...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ STEP 1: Clear old data before saving new data.
      // This is the crucial part to prevent the "old user" issue.
      cookie.remove('token');
      // If you're also using localStorage, clear it here as well:
      // localStorage.removeItem('user');

      // ✅ STEP 2: Save the new token and dispatch the new user info.
      cookie.save('token', token);
      dispatch({ type: 'login', payload: { username: returnedUsername, role } });
